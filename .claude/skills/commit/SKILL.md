---
name: commit
description: Create a clean git commit following conventional commits specification with English messages. Use when the user wants to commit changes, asks for a commit, or mentions "commit". ALSO trigger automatically after completing any unit task that successfully builds (compiles, passes tests, or passes linting) — run git status and if uncommitted changes exist, commit immediately without asking.
---
...
### Auto-commit mode (after a unit task that builds)
